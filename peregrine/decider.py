...
_SYSTEM_PROMPT = (
    ...
    "but you know the exact text to enter, put it in `text` and leave `value` null. Match the value to the "
    "target field's own label, not to the goal as a whole: a field labelled for one quantity must not receive "
    "another quantity's value, and a field that already shows the wanted value needs no retyping.\n"
    "- Prefer elements inside a dialog when one blocks the page. Do not repeat an action that `recent_actions` "
    "shows left the page unchanged, and do not reopen a control you have already opened; if a panel is open, "
    "act on the controls inside it instead.\n"
